...
import { MECHANICS, MATERIALS, SERVICES } from './storage.js';
import { showToast, showConfirm } from './utils.js';  // ← добавили, чтобы тосты и confirm работали
...
// Настройки (с кнопкой загрузки из 1С)
export function buildSettingsModal(state) {
  const body = document.getElementById('settingsBody');
  if (!body) return console.warn('Элемент #settingsBody не найден');
    ...
    <div style="margin-bottom: 20px; border-bottom: 1px solid var(--border); padding-bottom: 16px;">
      ...
      <div style="display: flex; gap: 8px;">
        <select id="subdivisionSelect" style="flex: 1; background: var(--surface-2); border: none; border-radius: var(--radius-sm); padding: 10px;">
          ...
          <!-- добавь остальные подразделения -->
        </select>
  ...
  // Добавь сюда отрисовку списков механиков, материалов, услуг (как в твоём старом коде)
  // Например:
  html += `<div class="settings-section"><div class="settings-section-title">Механики</div>`;
  MECHANICS.forEach(name => {
    html += `<div class="settings-item"><input type="text" value="${name}"><button class="settings-delete"><i class="fa-solid fa-trash-can"></i></button></div>`;
  });
  html += `<button class="settings-add-btn" id="addMech"><i class="fa-solid fa-plus"></i> Добавить механика</button></div>`;

  // ... аналогично для MATERIALS и SERVICES ...

  body.innerHTML = html;

  // Привязка кнопки загрузки из 1С
  document.getElementById('loadFrom1C')?.addEventListener('click', async () => {
    const subdivision = document.getElementById('subdivisionSelect')?.value;
    if (!subdivision) return showToast('Выберите подразделение');

    showToast('Загрузка настроек из 1С...');

    // Вызов функции загрузки (она должна быть в storage.js)
    await loadSettingsFrom1C(subdivision);  // ← добавь эту функцию в storage.js

    showToast('Настройки загружены');
    buildSettingsModal(state);  // перерисовываем модалку
  });
}
